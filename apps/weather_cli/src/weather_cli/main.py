...
class WeatherCLI:
    ...
    def display_weather(self, location, units="metric"):
        ...
        with Live(console=console, screen=True, redirect_stderr=False) as live:
            live.update(Panel("Fetching weather data...", title="[bold green]Weather Report[/bold green]"))
            ...
            if not current_weather or not forecast:
                live.update(Panel("[bold red]Could not retrieve weather data.[/bold red]", title="[bold red]Error[/bold red]"))
                return
    ...
    def _generate_weather_panel(self, weather_data, forecast_data):
        ...
        if weather_data.get("cod") != 200:
            return Panel(f"[bold red]Error:[/bold red] {weather_data.get('message', 'Could not fetch weather data.')}", title="[bold red]Error[/bold red]")
...
"""
    ...
    def _create_forecast_table(self, forecast_data):
        ...
        for date, summary in daily_summaries.items():
            ...
            table.add_row(date, f"{self._get_weather_emoji(most_common_weather_id)} {most_common_condition}", f"{min_temp:.1f}°C / {max_temp:.1f}°C")
            days_displayed += 1
...
def main():
    ...
    parser.add_argument("location", nargs="?", default=None, help="The city and country code to get weather for (e.g., 'London,uk').")
    parser.add_argument("--units", default="metric", choices=["metric", "imperial"], help="The units to display the temperature in.")
